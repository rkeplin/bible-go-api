...
type Repo interface {
	FindAll() (BookCollection, error)
	FindOne(id int) (Book, error)
	FindChapters(bookId int, translation string) (ChapterCollection, error)
}
...
func (h Handler) FindChapters(w http.ResponseWriter, r *http.Request) {
	vars := mux.Vars(r)
	id, err := strconv.Atoi(vars["id"])
	...
	collection, err := h.repo.FindChapters(id, translation)
